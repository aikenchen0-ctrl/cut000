...
function browserFailureSummary(result: unknown): string {
  ...
  return 'cut000 tool execution failed.';
}
...
function baseInstructions(request: CodexTurnRequest): string {
  const mode = request.askOnly
    ? [
        ...
        'You may call the provided read-only cut000 tools when current skill or project evidence is needed.',
      ].join(' ')
    : [
        'Use only the provided cut000 editor tools when the task needs project reads or changes.',
        'Their results come from the existing cut000 proposal workflow; never claim a change succeeded before its tool result confirms success.',
      ].join(' ');
  return [
    request.system.trim(),
    'cut000 runtime rules:',
...
export class CodexTurnManager {
  ...
  private serverRequest(session: TurnSession, request: CodexServerRequest): boolean {
    ...
    if (!callId || !name || !session.toolNames.has(name) || session.pendingTools.has(callId)) {
      const message = 'This cut000 tool call is unavailable. It was not part of this request (stale tool list, duplicate call, or malformed id). Tell the user to open the project and retry; if it persists, start a new run.'
  ...
  private abortPendingTools(session: TurnSession): void {
    for (const pending of session.pendingTools.values()) {
      pending.request.respond({
        contentItems: [{ type: 'inputText', text: 'cut000 cancelled this tool call.' }],
        success: false,
      });
    }
